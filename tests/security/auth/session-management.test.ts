...
mockSessionManager.createSession.mockImplementation((userId: string): Session => {
...
describe('Session Management Security Tests', () => {
  ...
  describe('Session Cookie Security', () => {
    ...
    it('should validate cookie security in different environments', () => {
      const cookieSecurityValidator = jest.fn((environment, cookieOptions) => {
        ...
        if (cookieOptions.maxAge > 24 * 60 * 60 * 1000) {
          // More than 24 hours
  ...
  describe('Session Fixation Protection', () => {
    it('should regenerate session IDs on privilege escalation', () => {
      ...
      const newSession = sessionFixationProtection.regenerateSessionId(
        initialSession.id,
        'user123'
      );
    ...
    it('should handle concurrent session regeneration safely', async () => {
      ...
      const regenerationPromise1 = concurrentRegenerationHandler.safeRegenerate(
        sessionId,
        'user123'
      );

      // Concurrent regeneration should fail
      const regenerationPromise2 = concurrentRegenerationHandler.safeRegenerate(
        sessionId,
        'user123'
      );

      await expect(regenerationPromise1).resolves.toBeDefined();
      await expect(regenerationPromise2).rejects.toThrow(
        'Session regeneration already in progress'
      );
    });
  });
  ...
  describe('Session Cleanup and Garbage Collection', () => {
    it('should clean up expired sessions automatically', () => {
      mockSessionManager.cleanupExpiredSessions.mockImplementation(() => {
        ...
        // Convert the Map entries to an array for safer iteration
        Array.from(mockSessionManager.sessions.entries()).forEach(([sessionId, session]) => {
    ...
    it('should implement session storage limits', () => {
      const sessionStorageManager = {
        ...
        enforceStorageLimits: jest.fn(function () {
          ...
          Array.from(sessionsByUser.entries()).forEach(([userId, userSessions]) => {
            if (userSessions.length > this.maxSessionsPerUser) {
              // Sort by last activity and remove oldest
              userSessions.sort(
                (a, b) =>
                  (a.session.lastActivity as Date).getTime() -
                  (b.session.lastActivity as Date).getTime()
              );

              const sessionsToRemove = userSessions.slice(
                0,
                userSessions.length - this.maxSessionsPerUser
              );
          ...
          if (mockSessionManager.sessions.size > this.maxConcurrentSessions) {
            const allSessions = Array.from(mockSessionManager.sessions.entries()).sort(
              ([, a], [, b]) =>
                (a.lastActivity as Date).getTime() - (b.lastActivity as Date).getTime()
            );

            const excessSessions = allSessions.slice(
              0,
              mockSessionManager.sessions.size - this.maxConcurrentSessions
            );
        ...
        suspiciousActivityDetectors: [
          {
            ...
            detect: (sessions, timeWindow = 5 * 60 * 1000) => {
              // 5 minutes
              const recentLogins = sessions.filter(
                (s) => Date.now() - s.createdAt.getTime() < timeWindow
              );
