...
global.securityTestUtils = {
  ...
  generateMaliciousInput: (type = 'sql') => {
    const payloads = {
    ...
    const typePayloads = payloads[type] || payloads.sql;
...
expect.extend({
  ...
  toBeSecurePassword(received) {
    ...
    const isSecure = received.length >= minLength && 
                    hasUppercase && 
                    hasLowercase && 
                    hasNumbers && 
                    hasSpecialChars;
    ...
    if (isSecure) {
      ...
      return {
        message: () => `Password does not meet security requirements: minimum ${minLength} characters, uppercase, lowercase, numbers, and special characters`,
        pass: false
      };
    }
  },
...
const performanceStartTimes = new Map();

global.startSecurityPerformanceTest = (testName) => {
  performanceStartTimes.set(testName, Date.now());
};

global.endSecurityPerformanceTest = (testName, maxDurationMs = 1000) => {
  ...
  if (duration > maxDurationMs) {
    throw new Error(`Security test "${testName}" took ${duration}ms, exceeding maximum of ${maxDurationMs}ms`);
  }
