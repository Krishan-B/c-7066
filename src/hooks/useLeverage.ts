...
import { leverageService, type LeverageCalculationResult } from '@/services/leverageService';
...
export const useLeverage = () => {
  ...
  const [marginCalculations, setMarginCalculations] = useState<MarginCalculation[]>([]);
  ...
  const updatePositionLeverage = useCallback(async (
    positionId: string,
    leverage?: number
  ): Promise<boolean> => {
    try {
      const success = await leverageService.updatePositionLeverage(positionId, leverage);
  ...
  const getLeverageConfig = useCallback(async (
    assetClass: string,
    symbol?: string
  ...
  const checkMarginCall = useCallback((marginLevel: number, marginCallLevel: number = 1.0) => {
    return leverageService.calculateMarginCallWarning(marginLevel, marginCallLevel);
  }, []);
