...
export async function fetchWatchlistData(options: WatchlistFetchOptions = {}): Promise<Asset[]> {
  try {
    console.warn('Fetching watchlist data');

    const { limit = 50, marketTypes = ['Stock', 'Index', 'Commodity', 'Forex', 'Crypto'] } =
      options;
    ...
    if (data && data.length > 0) {
      console.warn(`Found ${data.length} watchlist assets`);
      return data as Asset[];
    }

    // If no data in Supabase yet, call the functions to populate it
    console.warn('No watchlist data found, fetching fresh data');
    ...
    if (!refreshError && refreshedData && refreshedData.length > 0) {
      console.warn(`Fetched ${refreshedData.length} refreshed watchlist assets`);
