...
export async function updateAccountForTrade(
  ...
  try {
    console.log(`Updating account for user ${userId}, trade ${tradeId}, margin: ${marginRequired}`);
    ...
    if (newAvailableFunds < 0) {
      throw new Error(`Insufficient funds. Required: ${marginRequired}, Available: ${account.available_funds}`);
    }
    ...
    console.error('Error updating account for trade:', error);
    return { 
      success: false, 
...
export async function updateAccountForClosedTrade(
  ...
  try {
    console.log(`Updating account for closed trade ${tradeId}, release margin: ${releaseMargin}, P&L: ${profitLoss}`);
    ...
    console.error('Error updating account for closed trade:', error);
    return { 
      success: false, 
      message: error instanceof Error ? error.message : 'Unknown error occurred' 
    };
