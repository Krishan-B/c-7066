...
const HeroSection = () => {
  ...
  return <section className="py-16 md:py-24 container overflow-hidden">
      <div className="flex flex-col md:flex-row items-center gap-6 md:gap-12">
        <motion.div className="flex-1 space-y-6 md:space-y-8" initial={{
        opacity: 0,
        x: -20
      }} animate={{
        opacity: 1,
        ...
        delay: 0.2
          ...
          <div className="flex flex-wrap gap-3 md:gap-4 pt-2 md:pt-4">
            <Button size="lg" onClick={() => navigate("/auth?tab=signup")} className="whitespace-nowrap bg-primary hover:opacity-90 transition-all duration-300">
              Get Started
        ...
        <motion.div className="flex-1 flex justify-center w-full mt-8 md:mt-0" initial={{
        opacity: 0,
        x: 20
      }} animate={{
        opacity: 1,
        ...
        delay: 0.4
          ...
          <div className={`relative w-full max-w-lg h-[240px] md:h-[400px] rounded-lg overflow-hidden ${theme === 'dark' ? 'shadow-[0_0_30px_rgba(255,255,255,0.15)]' : 'shadow-lg'}`}>
            ...
            <img src="/trading-dashboard.svg" alt="Trading Dashboard" className="absolute inset-0 w-full h-full object-cover brightness-110 contrast-105" onError={e => {
            e.currentTarget.src = 'https://placehold.co/600x400/333333/8989DE?text=Trading+Dashboard';
          }} />
            
            <motion.div className={`absolute top-4 right-4 p-3 rounded-lg shadow-lg max-w-[180px] ${theme === 'dark' ? 'bg-white/10 backdrop-blur-md border border-white/20' : 'bg-card'}`} initial={{
            opacity: 0,
            y: 10
          }} animate={{
            opacity: 1,
