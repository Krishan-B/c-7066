...
const ProtectedRoute = ({ children }: ProtectedRouteProps) => {
  ...
  useEffect(() => {
    const attemptRefresh = async () => {
      if (!session && !loading && !refreshAttempted) {
        console.warn('No session detected, attempting to refresh');
        setIsRefreshing(true);

        try {
          const refreshedSession = await refreshSession();
          console.warn(
            'Session refresh attempt completed:',
  ...
  if (!session) {
    console.warn('No authenticated session found after refresh attempt, redirecting to auth page');
