...
const MarketHoursChart: React.FC<MarketHoursChartProps> = ({ marketType, className }) => {
  ...
  const data = Array.from({ length: 24 }, (_, i) => {
    ...
    const isTradingOvernight = spansOvernight && (i >= config.openTime || i < config.closeTime);
    ...
    <Card className={className}>
      ...
      <CardContent>
              ...
              <BarChart data={data} barGap={0} barCategoryGap={0}>
                ...
                <Bar dataKey="value" radius={0}>
                  {data.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={entry.isTrading 
                        ? (index === currentHour ? '#10B981' : '#6EE7B7') 
                        : '#e5e7eb'}
                    />
                  ))}
                </Bar>
        ...
        <div className="mt-4 text-xs text-muted-foreground">
          <p>
            {marketType} markets trade from {config.openTime}:00 to {config.closeTime}:00 UTC 
            on {config.openDays.map(day => {
