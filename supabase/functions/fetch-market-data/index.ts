...
// Mock market data function since we can't use real API keys here
const generateMarketData = (market: string) => {
  ...
  // Get the assets for the specified market
  const marketAssets = assets[market] || [];
  
  // Generate realistic fluctuations for each asset
  return marketAssets.map(asset => {
    ...
    const marketCap = (market === 'Stock' || market === 'Crypto') 
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // For each asset, upsert the data into the market_data table
